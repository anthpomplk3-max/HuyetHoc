...
# ========== DỮ LIỆU CÂU HỎI & ĐÁP ÁN (120 câu) - ĐÃ KIỂM TRA VÀ SỬA LỖI ==========
questions_data = {
    "Part 1": {
        1: {
    ...
    "Part 4": {
        ...
        # SỬA LỖI: Câu 14 - Đáp án đúng phải là D (Breadwinner), không phải B (Husband)
...
def shuffle_question_options(question):
    ...
    # Tạo ánh xạ từ nhãn gốc sang nhãn mới
    old_to_new = {}
    for i, old_key in enumerate(option_keys):
        # Tìm vị trí của option gốc trong permutation
        # old_key ở vị trí i, trong permutation tìm vị trí j sao cho permutation[j] = i
        # Khi đó new_keys[j] là nhãn mới
        for j in range(len(permutation)):
            if permutation[j] == i:
                old_to_new[old_key] = new_keys[j]
                break
    
    # Tạo ánh xạ từ mới sang cũ
    ...
    return {
        **question,
        ...
        "mapping": new_to_old,
        "reverse_mapping": old_to_new  # Thêm reverse_mapping để debug
    }
